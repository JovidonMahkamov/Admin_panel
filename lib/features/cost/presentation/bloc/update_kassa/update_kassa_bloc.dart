import 'dart:io';
import 'package:admin_panel/features/cost/domain/usecase/update_kassa_use_case.dart';
import 'package:admin_panel/features/cost/presentation/bloc/cost_event.dart';
import 'package:admin_panel/features/cost/presentation/bloc/update_kassa/update_kassa_state.dart';
import 'package:dio/dio.dart';
import 'package:flutter_bloc/flutter_bloc.dart';

class UpdateKassaBloc extends Bloc<CostEvent, UpdateKassaState> {
  final UpdateKassaUseCase updateKassaUseCase;

  UpdateKassaBloc(this.updateKassaUseCase) : super(UpdateKassaInitial()) {
    on<UpdateKassaE>(onLogInUser);
  }

  Future<void> onLogInUser(event, emit) async {
    emit(UpdateKassaLoading());
    try {
      final result = await updateKassaUseCase(
        id: event.id,
        doKon: event.doKon,
        izoh: event.izoh,
        mahsulotNomi: event.mahsulotNomi,
        sms: event.sms,
        summa: event.summa,
        tolovTuri: event.tolovTuri,
      );
      emit(UpdateKassaSuccess(updateCashExpenseEntity: result));
    } on DioException catch (e) {
      String errorMessage = _mapDioErrorToMessage(e);
      emit(UpdateKassaError( message: errorMessage));
    } catch (e) {
      emit(UpdateKassaError(message: "Noma’lum xato yuz berdi"));
    }
  }

  String _mapDioErrorToMessage(DioException error) {
    if (error.type == DioExceptionType.unknown &&
        error.error is SocketException) {
      return "Internet ulanmagan. Iltimos, tarmoqni tekshiring.";
    } else if (error.response?.statusCode == 400) {
      return "Kiritilgan kod xato";
    } else if (error.type == DioExceptionType.connectionTimeout ||
        error.type == DioExceptionType.receiveTimeout) {
      return "So‘rov vaqtida javob kelmadi. Keyinroq urinib ko‘ring.";
    } else if (error.response?.statusCode == 500) {
      return "Serverda nosozlik bor. Iltimos, keyinroq urinib ko‘ring.";
    }

    return "Noma’lum xato yuz berdi. Iltimos, qayta urinib ko‘ring.";
  }}